...
{-# LANGUAGE OverloadedStrings #-} --this language extension is required by CMQ
...
main = withSocketsDo $ do
     ...
     forM_ [0..10000] $ \i -> do
        cwPush qs ("192.168.35.69", 0) "ping" token --send message "ping" to
        --ipv4 address 192.168.35.69 using the queue specified in token
        cwPush qs ("192.168.35.85", 0) "pong" token
